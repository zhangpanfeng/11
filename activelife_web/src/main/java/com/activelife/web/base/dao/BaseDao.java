package com.activelife.web.base.dao;
...
public class BaseDao {
    ...
    protected <T> List<T> findEntitysById(String key, String id) throws BusinessException {
        List<T> result = null;
        try {
            result = this.sqlSession.selectList(this.formatKey(key), id);
        } catch (Exception e) {
            e.printStackTrace();
            throw new BusinessException(StatusCode.READ_ERROR, ErrorMessage.READ_ERROR("findEntitysById", id));
        }
    ...
    protected int getTotalCount(String key, Map<String, Object> params) throws BusinessException {
        int result;
        try {
            List<Object> list = this.sqlSession.selectList(this.formatKey(key));
